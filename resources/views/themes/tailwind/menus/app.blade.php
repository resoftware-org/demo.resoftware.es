...
<aside class="relative bg-sidebar h-screen w-96 hidden sm:block shadow-xl">
    <div class="p-6">
        <div class="flex flex-col items-center justify-between md:justify-start md:space-x-2">
            <div class="inline-flex">
                <a href="{{ route('wave.home') }}" class="flex items-center justify-center space-x-3 transition-all duration-1000 ease-out transform text-white">
                    @if(Voyager::image(theme('dark_logo')))
                        <img style="width: 100%;" src="{{ Voyager::image(theme('dark_logo')) }}" alt="Logo of {{config('site.title')}}">
                    @elseif(Voyager::image(theme('logo')))
                        <img style="width: 100%;" src="{{ Voyager::image(theme('logo')) }}" alt="Logo of {{config('site.title')}}">
                    @else
    ...
    <div class="absolute w-full bottom-2">
        @include("theme::partials.cta-upgrade")
    </div>
</aside>
