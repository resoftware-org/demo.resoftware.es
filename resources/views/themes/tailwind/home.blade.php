...
@section('content')
                ...
                <div class="flex flex-col items-start w-full mb-16 md:items-center lg:pr-12 lg:items-start lg:w-1/2 lg:mb-0">
                    ...
                    <div class="invisible w-full mt-5 transition-none duration-700 ease-out transform translate-y-12 opacity-0 delay-450 sm:mt-8 sm:flex sm:justify-center lg:justify-start sm:w-auto" data-replace='{ "transition-none": "transition-all", "invisible": "visible", "translate-y-12": "translate-y-0", "opacity-0": "opacity-100" }'>
                        ...
                        <div class="mt-3 sm:mt-0 sm:ml-3">
                            <a href="#features" class="flex items-center justify-center w-full px-8 py-3 text-base font-medium leading-6 text-indigo-700 transition duration-150 ease-in-out bg-indigo-100 border-2 border-transparent rounded-md hover:text-indigo-600 hover:bg-indigo-50 focus:outline-none focus:shadow-outline-indigo focus:border-indigo-300 md:py-4 md:text-lg md:px-10">
                                {{__('replay.guest.read_more')}}
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <section id="features" class="relative z-40 w-full pt-10 pb-16 lg:pt-5 xl:-mt-24 bg-gradient-to-b from-wave-500 via-wave-600 to-wave-400">
        ...
        <div class="relative z-20 flex flex-col items-start justify-start px-8 mx-auto sm:items-center max-w-7xl xl:px-5">
            <h2 class="text-4xl font-medium leading-9 text-white">{{__('replay.guest.features.title')}}</h2>
            <p class="mt-4 leading-6 sm:text-center text-wave-200">
                {{__('replay.guest.features.intro_p1')}}<br class="hidden md:block"> {{__('replay.guest.features.intro_p2')}}</p>
            ...
            <div class="grid mt-16 gap-y-10 sm:grid-cols-2 sm:gap-x-8 md:gap-x-12 lg:grid-cols-3 xl:grid-cols-4 lg:gap-20">
                @foreach(config('features') as $feature)
                    <div>
                        ...
                        <h3 class="mt-6 text-sm font-semibold leading-6 sm:text-center text-wave-100">{{ __($feature->title) }}</h3>
                        <p class="mt-2 text-sm leading-5 sm:text-center text-wave-200">{{ __($feature->description) }}</p>
                    </div>
                @endforeach
            </div>
            ...
            <div class="max-w-6xl px-10 pb-20 mx-auto bg-gray-100">
                <div class="flex flex-col items-center lg:flex-row">
                    <div class="flex flex-col justify-center w-full h-full mb-10 lg:pr-8 sm:w-4/5 md:items-center lg:mb-0 lg:items-start md:w-3/5 lg:w-1/2">
                        <p class="mb-2 text-base font-medium tracking-tight uppercase text-wave-500">{{__('replay.guest.testimonials.intro')}}</p>
                        <h2
                            class="text-4xl font-extrabold leading-10 tracking-tight text-gray-900 sm:leading-none lg:text-5xl xl:text-6xl"
                        >{{__('replay.guest.testimonials.title')}}</h2>
                        <p class="pr-5 my-6 text-lg text-gray-600 md:text-center lg:text-left">{{__('replay.guest.testimonials.description')}}</p>
                        <a
                            href="#_"
                        ...
                        >{{__('replay.guest.testimonials.view_case_studies')}}</a>
                    </div>
                    <div class="w-full sm:w-4/5 lg:w-1/2">
                        ...
                        @include("theme::partials.testimonial", [
                            ...
                            "message" => "Wave allowed me to build the Software as a Service of my dreams!",
                            "picture" => "themes/July2023/testimonial-1.png",
                        ])

                        @include("theme::partials.testimonial", [
                            "name" => "John Doe",
                            "company" => "CEO SomeCompany",
                            "message" => "Wave saved us hundreds of development hours. Creating a Software as a Service is now easier than ever with Wave.",
                            "picture" => "themes/July2023/testimonial-2.png",
                        ])

                        @include("theme::partials.testimonial", [
                            "name" => "John Smith",
                            "company" => "CEO SomeCompany",
                            "message" => "This is the best solution available for creating your own Software as a Service!",
                            "picture" => "themes/July2023/testimonial-3.png",
                        ])
    ...
    <div id="pricing" class="relative">
        ...
        <div class="relative z-20 px-8 pb-8 mx-auto max-w-7xl xl:px-5">
            <div class="w-full text-left sm:text-center">
                <h2 class="pt-12 text-4xl font-extrabold text-gray-900 lg:text-5xl">{{__('replay.guest.pricing.title')}}</h2>
                <p class="w-full my-1 text-base text-left text-gray-900 opacity-75 sm:my-2 sm:text-center sm:text-xl">{{__('replay.guest.pricing.intro')}}</p>
            </div>
